...
if (isset($_GET["network"]) && $auth) {
    ...
    while ($results !== false && ($res = $results->fetchArray(SQLITE3_ASSOC))) {
        ...
        $network_addresses = $db->query(
            "SELECT ip FROM network_addresses WHERE network_id = $id ORDER BY lastSeen DESC"
        );
...
if (isset($_GET['getAllQueries']) && $auth) {
    ...
    if ($_GET['getAllQueries'] !== "empty") {
        ...
        $dbquery =
            "SELECT timestamp, type, domain, client, status FROM queries WHERE timestamp >= :from AND timestamp <= :until ";
        if (isset($_GET["types"])) {
            $types = $_GET["types"];
        ...
        if (!is_bool($results)) {
            while ($row = $results->fetchArray()) {
                ...
                $allQueries[] = array($row[0], $query_type, utf8_encode($row[2]), utf8_encode($c), $row[4]);
            };
        }
    }
...
if (isset($_GET['topClients']) && $auth) {
    ...
    $stmt = $db->prepare(
        'SELECT client,count(client) FROM queries ' . $limit . ' GROUP by client order by count(client) desc limit 20'
    );
