...
function validDomainWildcard($domain_name)
{
    ...
    $validChars = preg_match(
        "/^((\*.)?[_a-z\d](-*[_a-z\d])*)(\.([_a-z\d](-*[a-z\d])*))*(\.([_a-z\d])*)*$/i",
        $domain_name
    );
    $lengthCheck = preg_match("/^.{1,253}$/", $domain_name);
    $labelLengthCheck = preg_match("/^[^\.]{1,63}(\.[^\.]{1,63})*$/", $domain_name);
...
function addStaticDHCPLease($mac, $ip, $hostname)
{
    ...
    try {
        ...
        foreach ($dhcp_static_leases as $lease) {
            if ($lease["hwaddr"] === $mac) {
                throw new Exception(
                    "Static release for MAC address (" . htmlspecialchars($mac) . ") already defined!<br>",
                    4
                );
            }
            if ($ip !== "noip" && $lease["IP"] === $ip) {
                throw new Exception(
                    "Static lease for IP address (" . htmlspecialchars($ip) . ") already defined!<br>",
                    5
                );
            }
            if ($lease["host"] === $hostname) {
                throw new Exception(
                    "Static lease for hostname (" . htmlspecialchars($hostname) . ") already defined!<br>",
                    6
                );
            }
        }
        ...
        case "DNS":
            ...
            if (isset($_POST["conditionalForwarding"])) {
                ...
                if (!validIP($conditionalForwardingIP)) {
                    $error .=
                        "Conditional forwarding IP (" .
                        htmlspecialchars($conditionalForwardingIP) .
                        ") is invalid!<br>";
                }
                ...
                if (!validDomain($conditionalForwardingDomain)) {
                    $error .=
                        "Conditional forwarding domain name (" .
                        htmlspecialchars($conditionalForwardingDomain) .
                        ") is invalid!<br>";
                }
                if (!$error) {
                    $addressArray = explode(".", $conditionalForwardingIP);
                    $reverseAddress =
                        $addressArray[2] . "." . $addressArray[1] . "." . $addressArray[0] . ".in-addr.arpa";
                    $extra .=
        ...
        case "API":
            ...
            foreach ($domains as $domain) {
                if (!validDomainWildcard($domain) || validIP($domain)) {
                    $error .=
                        "Top Domains/Ads entry " . htmlspecialchars($domain) . " is invalid (use only domains)!<br>";
                }
                if (!$first) {
                    $domainlist .= ",";
                } else {
            ...
            foreach ($clients as $client) {
                if (!validDomainWildcard($client) && !validIP($client)) {
                    $error .=
                        ...
                        " is invalid (use only host names and IP addresses)!<br>";
                }
                if (!$first) {
                    $clientlist .= ",";
                } else {
